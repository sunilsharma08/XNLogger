...
class NetworkInterceptorTests: XCTestCase {
    ...
    func testDataTaskURLRequestWithoutCompletionHandler() {
        ...
        for (index, session) in sessions.enumerated() {
            ...
            debugPrint("Start job at index \(index) for url = \(url[index])")
            session.dataTask(with: URLRequest(url: url[index])).resume()
            sleep(3)
            let status = TestUtils.isLogged(atPath: self.fileLogHandler!.currentPath,
                                            subString: url[index].absoluteString,
                                            numberOfTimes: 1)
    ...
    func testBackgroundURLSession() {
        ...
        // Create a URL for a web page to be downloaded.
        let url: [URL] = preTestData.urls
        let sessions: [URLSession] = [URLSession(configuration: .background(withIdentifier: "nlNetworkLoggerTest.background"))]
